...
class Proxy
{
    ...
    const uint32_t m_maxFrameTime{0U};
...
public:
    Proxy() = delete;
    ~Proxy();
    Proxy(const uint32_t maxGameLoops, const uint32_t maxRealGameTime, const BotConfig &botConfig, const uint32_t maxFrameLimit);
